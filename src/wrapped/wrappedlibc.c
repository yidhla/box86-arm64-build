...
EXPORT int32_t my_execv(x86emu_t* emu, const char* path, char* const argv[])
{
    ...
    if (x64 || x86 || script || self) {
        ...
        if(self)
            ...
            if(skip_first && (strstr(path, "wine-preloader") || strstr(path, "wine64-preloader"))) {
                char *wp = box_strdup(path);
                char *p = strstr(wp, "wine-preloader");
                if(!p) p = strstr(wp, "wine64-preloader");
                if(p) { strcpy(p, "wine"); newargv[toadd] = wp; }
...
EXPORT void* my_realpath(x86emu_t* emu, void* path, void* resolved_path)
{
    if(path && isProcSelf(path, "exe")) {
        return realpath(emu->context->argv[0], resolved_path);
    }
...
{
    if(isProcSelf(path, "exe")) {
        strncpy(buf, emu->context->argv[0], bufsize);
        size_t l = strlen(emu->context->argv[0]);
